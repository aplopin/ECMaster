...
int tab_pdo_map::si_map_sdo(int slave)
{
    ...
    ui->plainTextSlaveinfo->insertPlainText("PDO mapping according to CoE :\n");
    SMt_bug_add = 0;
    outputs_bo = 0;
    inputs_bo = 0;
        ...
        for (iSM = 2 ; iSM <= nSM ; iSM++)
        {
            ...
            if (wkc > 0)
            {
                if((iSM == 2) && (tSM == 2)) // SM2 has type 2 == mailbox out, this is a bug in the slave!
                {
                    ...
                    ui->plainTextSlaveinfo->insertPlainText("Activated SM type workaround, possible incorrect mapping.\n");
                }
                if(tSM)
                ...
                if (tSM == 3) // outputs
                {
                    ...
                    sprintf(str, "  SM%1d outputs\n     addr b   index: sub bitl data_type    name\n", iSM);
                    ui->plainTextSlaveinfo->insertPlainText(str);
    ...
    if (PDO->Startpos > 0)
    {
        ...
        do
        {
            ...
            if (PDO->SyncM[PDO->nPDO] < EC_MAXSM) /* active and in range SM? */
            {
                ...
                if (t)
                {
                    sprintf(str, "  SM%1d RXPDO 0x%4.4X %s\n", PDO->SyncM[PDO->nPDO], PDO->Index[PDO->nPDO], str_name);
                    ui->plainTextSlaveinfo->insertPlainText(str);
                }
                else
                {
                    sprintf(str, "  SM%1d TXPDO 0x%4.4X %s\n", PDO->SyncM[PDO->nPDO], PDO->Index[PDO->nPDO], str_name);
                    ui->plainTextSlaveinfo->insertPlainText(str);
                }
                ui->plainTextSlaveinfo->insertPlainText("     addr b   index: sub bitl data_type    name\n");
                ...
                for (er = 1; er <= e; er++)
                {
                    ...
                    if(obj_idx || obj_subidx)
                    {
                        ...
                        sprintf(str, "  [0x%4.4X.%1d] 0x%4.4X:0x%2.2X 0x%2.2X", abs_offset, abs_bit, obj_idx, obj_subidx, bitlen);
                        ui->plainTextSlaveinfo->insertPlainText(str);
...
int tab_pdo_map::si_map_sii(int slave)
{
    ...
    ui->plainTextSlaveinfo->insertPlainText("PDO mapping according to SII :\n");
...
void tab_pdo_map::si_sdo(int cnt)
{
    ...
    {
        sprintf(str, " CoE Object Description found, %d entries.\n",ODlist.Entries);
        ui->plainTextSlaveinfo->insertPlainText(str);
        for( i = 0 ; i < ODlist.Entries ; i++)
        {
            ...
            {
                sprintf(str, "0x%04x      %-40s      [%s]\n", ODlist.Index[i], name,
                       otype2string(ODlist.ObjectCode[i]));
                ui->plainTextSlaveinfo->insertPlainText(str);
            }
            else
            {
                sprintf(str, "0x%04x      %-40s      [%s  maxsub(0x%02x / %d)]\n",
            ...
            for( j = 0 ; j < max_sub+1 ; j++)
            {
                if ((OElist.DataType[j] > 0) && (OElist.BitLength[j] > 0))
                {
                    ...
                    sprintf(str, "    0x%02x      %-40s      [%-16s %6s]      ", j, name,
                    ...
                    {
                        sprintf(str, "%s", SDO2string(cnt, ODlist.Index[i], j, OElist.DataType[j]));
                        ui->plainTextSlaveinfo->insertPlainText(str);
                    }
                    ui->plainTextSlaveinfo->insertPlainText("\n");
                }
            }
        }
    }
...
void tab_pdo_map::slaveinfo(char *ifname)
{
    ...
    ui->plainTextSlaveinfo->insertPlainText("Starting slaveinfo\n");

    /* initialise SOEM, bind socket to ifname */
    if (ec_init(ifname))
    {
        sprintf(str, "ec_init on %s succeeded.\n",ifname);
        ui->plainTextSlaveinfo->insertPlainText(str);
        ...
        if ( ec_config(FALSE, &IOmap) > 0 )
        {
            ...
            while(EcatError)
            {
                sprintf(str, "%s", ec_elist2string());
                ui->plainTextSlaveinfo->insertPlainText(str);
            }
            sprintf(str, "%d slaves found and configured.\n",ec_slavecount);
            ui->plainTextSlaveinfo->insertPlainText(str);
            expectedWKC = (ec_group[0].outputsWKC * 2) + ec_group[0].inputsWKC;
            sprintf(str, "Calculated workcounter %d\n", expectedWKC);
            ui->plainTextSlaveinfo->insertPlainText(str);
            /* wait for all slaves to reach SAFE_OP state */
            ec_statecheck(0, EC_STATE_SAFE_OP,  EC_TIMEOUTSTATE * 3);
            if (ec_slave[0].state != EC_STATE_SAFE_OP )
            {
                sprintf(str, "Not all slaves reached safe operational state.\n");
                ui->plainTextSlaveinfo->insertPlainText(str);
                ec_readstate();
                for(i = 1; i<=ec_slavecount ; i++)
                {
                    ...
                    {
                        sprintf(str, "Slave %d State=%2x StatusCode=%4x : %s\n",
            ...
            for( cnt = 1 ; cnt <= ec_slavecount ; cnt++)
            {
                ...
                {
                    sprintf(str, " DCParentport:%d\n", ec_slave[cnt].parentport);
                    ui->plainTextSlaveinfo->insertPlainText(str);
                }
                sprintf(str, " Activeports:%d.%d.%d.%d\n", (ec_slave[cnt].activeports & 0x01) > 0 ,
                       (ec_slave[cnt].activeports & 0x02) > 0 ,
                       (ec_slave[cnt].activeports & 0x04) > 0 ,
                       (ec_slave[cnt].activeports & 0x08) > 0 );
                ui->plainTextSlaveinfo->insertPlainText(str);
                sprintf(str, " Configured address: %4.4x\n", ec_slave[cnt].configadr);
                ui->plainTextSlaveinfo->insertPlainText(str);
                sprintf(str, " Man: %8.8x ID: %8.8x Rev: %8.8x\n", (int)ec_slave[cnt].eep_man, (int)ec_slave[cnt].eep_id, (int)ec_slave[cnt].eep_rev);
                ui->plainTextSlaveinfo->insertPlainText(str);
                for(nSM = 0 ; nSM < EC_MAXSM ; nSM++)
                {
                    if(ec_slave[cnt].SM[nSM].StartAddr > 0)
                    {
                        sprintf(str, " SM%1d A:%4.4x L:%4d F:%8.8x Type:%d\n",nSM, etohs(ec_slave[cnt].SM[nSM].StartAddr), etohs(ec_slave[cnt].SM[nSM].SMlength),
                ...
                {
                    sprintf(str, " FMMU%1d Ls:%8.8x Ll:%4d Lsb:%d Leb:%d Ps:%4.4x Psb:%d Ty:%2.2x Act:%2.2x\n", j,
                ...
                sprintf(str, " FMMUfunc 0:%d 1:%d 2:%d 3:%d\n",
                       ec_slave[cnt].FMMU0func, ec_slave[cnt].FMMU1func, ec_slave[cnt].FMMU2func, ec_slave[cnt].FMMU3func);
                ui->plainTextSlaveinfo->insertPlainText(str);
                sprintf(str, " MBX length wr: %d rd: %d MBX protocols : %2.2x\n", ec_slave[cnt].mbx_l, ec_slave[cnt].mbx_rl, ec_slave[cnt].mbx_proto);
                ui->plainTextSlaveinfo->insertPlainText(str);
                ...
                sprintf(str, " CoE details: %2.2x FoE details: %2.2x EoE details: %2.2x SoE details: %2.2x\n",
                       ec_slave[cnt].CoEdetails, ec_slave[cnt].FoEdetails, ec_slave[cnt].EoEdetails, ec_slave[cnt].SoEdetails);
                ui->plainTextSlaveinfo->insertPlainText(str);

                sprintf(str, " Ebus current: %d[mA]\n only LRD/LWR:%d\n",
        ...
        {
            ui->plainTextSlaveinfo->insertPlainText("No slaves found!\n");
        }
        ui->plainTextSlaveinfo->insertPlainText("\nRequest init state for all slaves\n");
        ec_slave[0].state = EC_STATE_INIT;
        /* request INIT state for all slaves */
        ec_writestate(0);
        ui->plainTextSlaveinfo->insertPlainText("End slaveinfo, close socket\n");
        /* stop SOEM, close socket */
        ec_close();
    }
    else
    {
        sprintf(str, "No socket connection on %s\nExcecute as root\n",ifname);
        ui->plainTextSlaveinfo->insertPlainText(str);
    }
}
...
void tab_pdo_map::slaveinfo_start(void)
{
    ...
    ui->plainTextSlaveinfo->insertPlainText("Information about all connected EtherCAT slaves\n");

    ui->plainTextSlaveinfo->insertPlainText("Available adapters\n");
    adapter = ec_find_adapters ();
    while (adapter != NULL)
    {
        sprintf(str, "Description : %s, Device to use for wpcap: %s\n", adapter->desc,adapter->name);
        ui->plainTextSlaveinfo->insertPlainText(str);
